...
└────────┬────────────┘           │                       │
         ...
         ▼                        │                       │
┌─────────────────────┐           │                       │
│ Step 2: Idea Gen    │◄──────────┤  PIVOT: all ideas     │
│ (Student, Sonnet)   │           │  dead, budget remains │
│                     │           │                       │
│ READ: landscape     │           │                       │
│ WebSearch to verify │           │                       │
│ Generate 10+ ideas  │           │                       │
...
└────────┬─────────────────────────┘                      │
         │                                                │
         │  Diversity Gap? ──yes──► back to Step 2        │
         │                         (retry ≤5x total)      │
         │                                                │
         ├── DEVELOP ideas ──────────────┐                │
         │                               │                │
...
└────────┬──────────────────────────────────┘                 │
         ...
         │ NO + can_pivot → PIVOT ─────────┼──────┼────────────┘
         │ NO + no budget → INFEASIBLE     │      │ (back to Step 2)
         │ YES → continue                  │      │
         └────────┬───────────────────────┘      │
                  │                               │
                  ▼                               │
         ...
         │                                 │      │
         │ ≥3 approved + all severe/major  │      │
         │ resolved? ──yes──► Stage 9 ─────┤      │
         │                                 │      │
         │ Not converged?                  │      │
         │ start_new_round() (N++)         │      │
         │ quality: lenient→moderate→strict │      │
